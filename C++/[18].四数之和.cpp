...
=========================================*/
/*=======================================
                (题目难度：中等)  官方答案有bug，测试用例通不过
给你一个由 n 个整数组成的数组 sums ，和一个目标值 target
。请你找出并返回满足下述全部条件且不重复的四元组 [sums[a], sums[b], sums[c],
sums[d]] ：
  ...
  static vector<vector<int>> fourSum(vector<int> &nums, int target) {
    ...
    for (int i = 0; i < length - 3; ++i) {
      ...
      if (nums[i] + nums[i + 1] > target - nums[i + 2] - nums[i + 3]) {
        break; // 已经从小到大排了序的四个数字之和都大于 target
               // ，说明没有符合要求的四元数组
      }
      if (nums[i] + nums[length - 3] <
          target - nums[length - 2] -
              nums[length -
                   1]) { // 从小到大排序，如果第一个和最后面三个数相加都比
                         // target 小，说明没有符合要求的四元数组
      ...
      for (int j = i + 1; j < length - 2; ++j) {
        ...
        int left = j + 1,
            right =
                length - 1; // 定义双指针,第三和第四个数字,代表了最左边和最右边
        while (
            left <
            right) { // 左指针要比有指针小，左右指针是逻辑上的概念，左右指针定义了左指针比右指针小
          long long sum =
              (long long)nums[i] + nums[j] + nums[left] + nums[right];
          if (sum == target) {
            vector<int> nums_list({nums[i], nums[j], nums[left], nums[right]});
            quadruplets.push_back(nums_list);
...
int main(int argc, char *argv[]) {
  vector<int> b({1, -2, -5, -4, -3, 3, 3, 5});
  int i2 = -11;
  vector<vector<int>> points = Solution::fourSum(b, i2);
  for (vector<vector<int>>::iterator it = points.begin(); it < points.end();
       ++it) {
    for (vector<int>::iterator itit = (*it).begin(); itit < (*it).end();
         ++itit) {
      cout << *itit << " ->" << ends;
    }
    cout << endl;
